...
for jfreq in [2]:
    ...
    for ifov in range(0,ifov_end):
        for iscan in range(0,iscan_end):
            ...
            if footprint_type == 'source':
                if (ifov == 0):
                    cross_scan_vector = 0.5*np.array([lons[iscan,1] - lons[iscan,0],
                                                  lats[iscan,1] - lats[iscan,0]])
                elif (ifov == ifov_end-1):
                    cross_scan_vector = 0.5*np.array([lons[iscan,ifov_end-1] - lons[iscan,ifov_end-2],
                                                  lats[iscan,ifov_end-1] - lats[iscan,ifov_end-2]])
                else:
                    cross_scan_vector = 0.25*np.array([lons[iscan,ifov+1] - lons[iscan,ifov-1],
                                                      lats[iscan,ifov+1] - lats[iscan,ifov-1]])
                gain_tot = np.zeros((1601,2401))
                for smear_factor in [-0.9,-0.6,-0.3,0.0,0.3,0.6,0.9]:
                    xloc,yloc = grid.projection(lons[iscan,ifov]+smear_factor*cross_scan_vector[0],
                                                lats[iscan,ifov]+smear_factor*cross_scan_vector[1])
