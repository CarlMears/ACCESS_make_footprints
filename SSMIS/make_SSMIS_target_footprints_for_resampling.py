...
def write_footprint_locations(lats,lons,ksat,footprint_type,band = 2):
    import os

    if footprint_type == 'target':
        textfile = Path(f'L:/access/resampling/SSMIS/f{ksat:02d}/target_gains/locs/find_target_gain_f{ksat:02d}_circular.loc')
...
def add_extra_locations(lats,lons,azim,extra_fovs,extra_scans):
    ...
    if extra_fovs > 0:
        for jscan in [0,num_output_scans-1]:
    ...
    if extra_scans > 0:
        for jscan in range(1,extra_scans+1):
            jscan_in = jscan_in = int(jscan/(1+extra_scans))
            wt_upper = (jscan - (jscan_in*(1+extra_scans)))/(1+extra_scans)
            wt_lower = 1.0-(wt_upper)

            lons_out[jscan,:] = wt_mean_angle([lons_out[0,:],lons_out[extra_scans+1,:]],np.array((wt_lower,wt_upper)))
            lats_out[jscan,:] = wt_mean_angle([lats_out[0,:],lats_out[extra_scans+1,:]],np.array((wt_lower,wt_upper)))
            azim_out[jscan,:] = wt_mean_angle([azim_out[0,:],azim_out[extra_scans+1,:]],np.array((wt_lower,wt_upper)))
...
for ifreq,freq in enumerate(freq_list):
    ...
    output_path_binary = Path(f'L:/access/resampling/SSMIS/f{ksat}/target_gains/{target_size:02d}km/')
    output_path_plot = Path(f'L:/access/resampling/SSMIS/f{ksat}/target_gains/{target_size:02d}km/plots/') 
    ...
    lons = lons[scan0:scan0+2,:]-lons[scan0,int(num_cells/2)]
    ...
    write_footprint_locations(lats[0:extra_scans+1,:],lons[0:extra_scans+1,:],ksat,footprint_type,band = jfreq)
    ...
    for jscan in np.arange(0,num_scans-1):
        for jfov in np.arange(0,num_fovs):
            ...
            if make_plots:
                if ((jfov%40 == 3) and (jscan == 0)):
                    grid.adddata(gain,name='Sum')
            
            output_path_binary2 = output_path_binary / f'band_{jfreq:02d}/'
            os.makedirs(output_path_binary2,exist_ok = True)
            target_file_name = output_path_binary2 /f's{jscan+1:02d}c{jfov+1:03d}.dat'
            grid.write_binary(outfile = target_file_name,name='Single',save_thres=0.0001) 
...
#plot it
    if make_plots:
        ...
        fig0,ax,im =grid.contourplot(name='Sum',
                        ...
                        outfile = outfile)
        plt.show()
        png_file = output_path_plot / f'overall_scan_{freq}.png'
