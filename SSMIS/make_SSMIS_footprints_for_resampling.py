...
def write_footprint_locations(lats,lons,ksat,footprint_type,band = 2):
    ...
    if footprint_type == 'target':
        textfile = Path(f'L:/access/resampling/SSMIS/target_gains/locs/find_target_gain_f{ksat:02d}_circular_30km.loc')
    elif footprint_type == 'source':
        textfile = Path(f'L:/access/resampling/SSMIS/source_gains/locs/find_source_gain_f{ksat:02d}_band_{band:02d}_locs.txt')
    else:
...
for ifreq,freq in enumerate(freq_list):
    ...
    write_footprint_locations(lats,lons,ksat,footprint_type,band = 2)
