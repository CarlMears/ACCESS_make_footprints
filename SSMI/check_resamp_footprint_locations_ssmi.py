...
if __name__ == '__main__':
    ...
    for scan in np.arange(0,num_scans):
        for fov in np.arange(0,num_fovs):
            resamp_gain,iy_max_resamp,ix_max_resamp = read_gain_array(sat_name=sat_name,
                                                                ksat=ksat,
                                                                beamwidth=beamwidth,
                                                                band_num=freq_num,
                                                                kscan=scan+1,
            ...
            print(f'{scan+1:02d},{fov+1:03d} iy: {iy_proj:.2f}, {iy_max_resamp:.2f}, ix:{ix_proj:.2f}, {ix_max_resamp:.2f}')
            ix_diff[scan,fov] = ix_proj - ix_max_resamp
            iy_diff[scan,fov] = iy_proj - iy_max_resamp
