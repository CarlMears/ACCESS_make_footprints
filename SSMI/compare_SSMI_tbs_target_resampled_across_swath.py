...
band_list = [0,1,2]
band_names = ['19GHz','22GHz','37GHz']
scan_list = [1,2,3,4]
num_fovs = 253


rms_diff_array = np.zeros((len(scene_list),len(band_list),len(scan_list),num_fovs),dtype = np.float32)
mean_diff_array = np.zeros((len(scene_list),len(band_list),len(scan_list),num_fovs),dtype = np.float32)
...
for scene_index,scene in enumerate(scene_list):
    ...
    for band_index,band_num in enumerate(band_list):
        fig,ax = plt.subplots()
        for ikscan,kscan in enumerate(scan_list):
            for fov in range(1,num_fovs+1):
                ...
                rms_diff_array[scene_index,band_index,ikscan,fov-1] = np.sqrt(np.mean(np.square(diff_list)))
                mean_diff_array[scene_index,band_index,ikscan,fov-1] = np.mean(diff_list)

                ax.plot(np.arange(1,num_fovs+1),rms_diff_array[scene_index,band_index,ikscan,:],label=f'{band_names[band_num]}, Scan {kscan}')
        ...
        ax.set_title(scene)
        ax.legend()
        png_file = Path(f'L:/access/resampling/{sat_name}/f{ksat:02d}/eval/shape_errors/Shape_Error_{scene}_{beamwidth}km_{band_names[band_num]}.png')
...
rms_error = xr.Dataset(
                data_vars=dict(
                    rms_error=(['scenes','bands','scans','fovs'], rms_diff_array),
                    mean_error=(['scenes','bands','scans','fovs'], mean_diff_array),
                ),
                coords=dict(
                    scenes=(["scenes"],np.arange(0,len(scene_list))),
                    bands=(["bands"],np.array(band_list)),
                    scans=(["scans"],np.array(scan_list)),
...
nc_file = Path(f'L:/access/resampling/{sat_name}/f{ksat:02d}/eval/shape_errors/Shape_Error_{beamwidth}km.nc')
