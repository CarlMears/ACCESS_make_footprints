...
def read_debug_file(sat_name='SSMI',ifreq=4):
...
def read_location_file(sat_name='SSMI',ksat=13,beamwidth=70):
...
if __name__ == '__main__':
    ...
    target_lats,target_lons = read_location_file(sat_name='SSMI',beamwidth=30)
    ...
    stddev_pattern_diff = np.zeros((253,4,4)) #fov,kscan,band
    tot_pattern_diff = np.zeros((253,4,4))

    plot_figs = True
    freq_names = ['19GHz','24GHz','37GHz']
    
    
    for kscan in [1,2,3,4]:
        for fov in [1,5,11,20,21,31,41,125,126,127,128]:
            ...
            if plot_figs:
                ...
                target_fig,target_ax,target_im = plot_2d_array(target_sub_to_plt, xvals, yvals,zrange=(-1.0,1.0),
                                                     title=f'Target Pattern, FOV = {fov:03d}, KSCAN = {kscan:02d}', 
            ...
            if plot_figs:
                ...
                png_path = f'L:/access/resampling/SSMI/f{ksat:02d}/resample_gains/{beamwidth:02d}km/plots/'
    ...
    # png_path = Path(f'L:/access/resampling/SSMI/resample_gains_v3/circular_{beamwidth:02d}km/plots/')
