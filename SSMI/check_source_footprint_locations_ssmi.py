...
if __name__ == '__main__':
    ...
    beamwidth = 70
    freq_num = 1
    source_lats,source_lons,_ = read_location_file(sat_name=sat_name,
                                                 ksat=ksat,
                                                 beamwidth=beamwidth,
                                                 freq=freq_num,
    ...
    num_scans = source_lats.shape[0]
    num_fovs = source_lats.shape[1]
    ...
    for scan in np.arange(0,num_scans):
        for fov in np.arange(0,num_fovs):
            source_gain,iy_max_source,ix_max_source = read_gain_array(sat_name=sat_name,
                                                                ksat=ksat,
                                                                beamwidth=beamwidth,
                                                                band_num=freq_num,
                                                                kscan=scan+1,
            ...
            z = grid.projection(source_lons[scan,fov],source_lats[scan,fov])
            ix_proj = z[0]/1000.0 + 1200.0
            iy_proj = z[1]/1000.0 + 800.0

            print(f'{scan+1:02d},{fov+1:03d} iy: {iy_proj:.2f}, {iy_max_source:.2f}, ix:{ix_proj:.2f}, {ix_max_source:.2f}')
            ix_diff[scan,fov] = ix_proj - ix_max_source
            iy_diff[scan,fov] = iy_proj - iy_max_source
